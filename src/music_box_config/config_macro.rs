...
macro_rules! config_macro_list_items {
    ($enclosing:ty, $($item:ident),+ $(,)?) => {
        pub fn get_items() -> Vec<(String, String, String, ValueType, String)> {
            paste::paste! {
                let items = vec![
                    $(
        ...
        $(
            paste::paste! {
                fn [<get_ $item _name>] () -> (String, String, String, ValueType, String) {
                    return ($group.to_string(), stringify!($item).to_string(), $item_string.to_string(), $value_type_enum, $help.to_string());
                }
            }
        )+
    };
